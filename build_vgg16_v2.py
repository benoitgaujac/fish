...
def build_model(input_var,nclasses,GPU=False):
    ...
    ################### ST1 ###################
    b1 = np.zeros((2, 3), dtype=theano.config.floatX)
    b1[0, 0] = 1
    b1[1, 1] = 1
    b1 = b1.flatten()
    """
    loc1_l2 = Conv2DLayer(
                ...
                W=HeUniform('relu'),
                nonlinearity=elu)
    loc1_l3 = MaxPoolLayer(loc_l2, pool_size=(2, 2))
    loc1_l4 = Conv2DLayer(
                loc_l3,
                num_filters=32,
                filter_size=(5, 5),
    ...
    loc1_out = DenseLayer(
                ...
                nonlinearity=None)
    # Transformer network
    l_trans1 = TransformerLayer(pool1, loc1_out, downsample_factor=(1.0,1.0))
    ...
    ################### ST2 ###################
    b2 = np.zeros((2, 3), dtype=theano.config.floatX)
    b2[0, 0] = 1
    b2[1, 1] = 1
    b2 = b2.flatten()
    """
    ...
    loc2_out = DenseLayer(
                ...
                nonlinearity=None)
    # Transformer network
    l_trans2 = TransformerLayer(pool2, loc2_out, downsample_factor=(1.0,1.0))
