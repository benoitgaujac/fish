...
parser.add_option('-s', '--fromscratch', action='store', dest='fromscratch',
    help="training from VGG weights or from own pretrained weights [vgg,own]")
...
def load_params(network,weight_path,from_scratch):
...
def main(from_scratch=True, GPU=False, num_epochs=50) :
    ...
    while epoch<num_epochs:
        ...
        for batch in train_data_set.iterate_minibatches(train_data_set.train_set,testing=False):
        ...
        val_batches = 0
        val_acc = 0
        for batch in train_data_set.iterate_minibatches(train_data_set.val_set,testing=False):
            inputs, targets, _ = batch
...
if __name__ == '__main__':
    ...
    # mode test or train
    main(options.fromscratch=="vgg",options.ressources=="GPU")
